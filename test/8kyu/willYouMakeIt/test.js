...
const zeroFuel = require("../../../8kyu/willYouMakeIt/solution");

describe("willYouMakeIt zeroFill", function() {
  it("Sample Tests", function() {
    assert.equal(zeroFuel(50, 25, 2), true);
    assert.equal(zeroFuel(100, 50, 1), false);
  });
  it("willYouMakeIt Basic Tests", function() {
    assert.equal(zeroFuel(60, 30, 3), true);
    assert.equal(zeroFuel(70, 25, 1), false);
    assert.equal(zeroFuel(100, 25, 3), false);
  });
  it("willYouMakeIt Random Tests", function() {
    for (let i = 0; i < 100; ++i) {
